...
def run_python(containerId,timelimit):
    print("container allocated ",CONTAINER_NAME,containerId)
    # cmd = f"python3 {pyCodeFile}"  #1
    cmd  = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout {timelimit}s  python3 src/pyCode.py < src/input.txt'"
...
def run_cpp(containerId,timelimit):
    ...
    if subprocessCppExe.returncode == 0:
        ...
        ExeCmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout {timelimit}s  src/./cppExeFile < src/input.txt'"
...
def run_c(containerId,timelimit):
    ...
    if subprocessCExe.returncode == 0:
        ...
        ExeCmd = f"docker exec {CONTAINER_NAME} sh -c 'timeout {timelimit}s  src/./cExeFile < src/input.txt'"
        subprocessOutput = subprocess.Popen(
            ExeCmd,
            shell=True,
            # preexec_fn=setLimit(),
...
def run_java(containerId,timelimit):
    # cmd = "gcc " + f"{cCodeFile}" + f" -o {directoryName}/cExeFile"
    cmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'javac src/Solution.java'"
    subprocessCExe = subprocess.Popen(cmd, shell=True, stderr=errorFile)
    subprocessCExe.wait()
    if subprocessCExe.returncode == 0:
        # ExeCmd = r"{0}/./cExeFile".format(directoryName)
        subprocess.run(['cp', '/src/Solution.class', '/'], shell=True)

        ExeCmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout {timelimit}s java Solution < src/input.txt'"
        # ExeCmd = f"docker exec {CONTAINER_NAME}{containerId} sh -c 'timeout {timelimit}s java -cp . src.Solution < src/input.txt'"
        subprocessOutput = subprocess.Popen(
            ExeCmd,
            shell=True,
            # preexec_fn=setLimit(),
