...
# from .tasks import delete_container
from contest import models as modelC
from django.conf import settings
from django.core.cache import cache
from .judgeUtils import deleteContainerById
...
def getInputLists(question):
    TCs = Testcase.objects.filter(question=question).order_by('testcaseNumber')
    inputList =[]
    for testcase in TCs:
        with open(testcase.inputFile.path, 'rb') as ip_file:
            ip_content = ip_file.read()

        with open(testcase.outputFile.path, 'rb') as op_file:
            op_content = op_file.read()

        inputTuple = (ip_content, op_content)
...
def getInputListsWithCache(question):
    ...
    cache_key = f"testcases_{question.questionId}"  # Assuming 'question' is a Django model instance
    ...
    cache.set(cache_key, inputList, timeout=60*60*3)  # Cache for 1 hour (3600 seconds)
...
#     delete_container.delay(containerObj.id)
...
def getOneStatus(comipledObj):
    ...
    if(status_string == 'AC' or status_string == 'WA'):
        opDict ={
            'status':'AC',
            'output':singleTestCaseResult[1][0],
            'error':singleTestCaseResult[1][1]
        }
    else:
        rError = singleTestCaseResult[1][1] 
        if status_string == 'CE':
            rError=comipledObj[1]
        opDict ={
