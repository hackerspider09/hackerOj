...
//to get input to different comp
  ...
  return (
    <div className="chaljabhai" id="test-cases">
      ...
      <h4 className='heading'> Console Window </h4>
    ...
    <div className="Status">
      {/* <div className="overallStatus">
       ...
       <div className="alltext d-flex flex-wrap ">
          ...
          {code?.status && Object.entries(code.status).map(([testCaseName, testCaseData]) => (
                  

                  <div className="test d-flex mx-3" key={testCaseName}  style={{backgroundColor: statusColors[testCaseData.status] }}>
                    {testCaseName} &nbsp; {testCaseData} &nbsp; {testCaseData === "AC" ? <BsCheck2Circle/> : <RxCrossCircled/>}
                  </div>
                ))}
          </div> 
